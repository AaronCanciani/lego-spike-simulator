...
The front attachment has paired steep bent beams, a connecting knuckle and one narrow toothed lifting finger. The rear attachment has two broad quarter-arch rails (16 solid segments per rail), a wide blue blade with molded-face details and a purple bottom edge. The open space between the rails is preserved. Both shapes follow the user's assembly pictures more closely than the original thin sticks and flat paddle.

Rendering and collision use the **same main part outlines and orientations**. Rounded beam ends and visible pin holes are modeled in the mesh but use conservative solid-box collision envelopes; the simulator does not allow threading objects through pin holes. Rack teeth, hole rims, blade recesses and the colored mounting hardware are cosmetic. The rectangular mounting brackets stay fixed to the chassis; the visible tan gears turn with the attachment's hinge angle (not a separately simulated gear train). Mount snapshots are explicitly marked `decorative` and do not add physics bodies or mass. These are original procedural approximations, not imported LEGO CAD or an exact brick assembly. The user-requested reduced dimensions remain unchanged.
...
`attachments.test.mjs` checks front/rear placement, matching main-part collision envelopes, independent motor targets, gearing and reverse polarity, finite travel stops, native Scratch C/D commands, unloaded motors, blade contact versus raised misses, and supported lifting versus missed pickup. It also checks shared-season worlds, sensor rays and interaction with a non-selected mission. `attachmentView.test.mjs` checks curved rail continuity, the narrow rack finger, finite detail meshes, fixed mounting frames, animated gears and cleanup. These establish implementation behavior, not real-hardware equivalence.
